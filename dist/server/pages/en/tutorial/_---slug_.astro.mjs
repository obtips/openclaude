...
async function getStaticPaths() {
  const tutorials = await getCollection("tutorial-en");
...
const $$ = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "TutorialLayout", $$Tutorial, { "title": title, "description": description, "lang": "en" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="mb-8"> <div class="flex items-center gap-2 mb-4"> <span${addAttribute(`text-xs px-2 py-1 rounded-full ${entry.data.difficulty === "beginner" ? "bg-green-100 text-green-700" : entry.data.difficulty === "intermediate" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`, "class")}> ${entry.data.difficulty} </span> ${entry.data.date && renderTemplate`<span class="text-sm text-claude-text-light"> ${entry.data.date.toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })} </span>`} </div> <h1 class="text-3xl md:text-4xl font-bold mb-4">${title}</h1> <p class="text-lg text-claude-text-light">${description}</p> ${entry.data.tags && entry.data.tags.length > 0 && renderTemplate`<div class="flex gap-2 mt-4"> ${entry.data.tags.map((tag) => renderTemplate`<span class="text-sm px-2 py-1 rounded bg-claude-warm text-claude-text-light">
#${tag} </span>`)} </div>`} </div> ${renderComponent($$result2, "Content", Content, {})} ` })}`;
}, "/Users/macadmin/Dev/openclaude/src/pages/en/tutorial/[...slug].astro", void 0);

const $$file = "/Users/macadmin/Dev/openclaude/src/pages/en/tutorial/[...slug].astro";
const $$url = "/en/tutorial/[...slug]";
